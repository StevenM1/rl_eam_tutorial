...
\node[box, text width=4.0cm] (data) {Data};
\node[box, text width=4.0cm, right=1cm of data] (covariates) {Covariates};
\node[box, text width=4.0cm, right=1cm of covariates] (deltarule) {Delta rule};

% arrows
\draw[->] (data) -- (covariates);
\draw[->] (covariates) -- (deltarule);

% verbatim-ish tables underneath
\node[vblock, text width=5.0cm, below=0.25cm of data] (data_tab) {
...
\node[vblock, text width=5.0cm, below=0.25cm of covariates] (covariates_tab) {
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily]
sym_D sym_e sym_T sym_t
    1    NA    NA    NA
   NA    NA    NA     0
    0    NA    NA    NA
    1    NA    NA    NA
...
\node[vblock, text width=5.0cm, below=0.25cm of deltarule] (deltarule_tab) {
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily]
sym_D sym_e sym_T sym_t
0.000     0   0.0     0
0.200     0   0.0     0
...
% more arrows
\draw[->] (data_tab) -- (covariates_tab);
\draw[->] (covariates_tab) -- (deltarule_tab);
...
symcolns <- c('sym_D', 'sym_e', 'sym_T', 'sym_t')
tmp <- emc[[1]]$data[[1]][,c('S', 'R', 'rt', 'reward', 's_left', 's_right', 'sym_D', 'sym_e', 'sym_T', 'sym_t')] # 1, 3, 2, 7
tmp$rt <- round(tmp$rt,3) 
rowidx1 <- tmp$s_left%in% c('sym_D', 'sym_e', 'sym_T', 'sym_t')
tmp <- tmp[rowidx1,]
tmp[seq(1, 12, 2),]

kernel_pars <- c('v.q0'=0, 'v.alpha'=0.2)
tmp2 <- apply_kernel(kernel_pars, emc = emc, subject=1)
tmp2 <- tmp2[rowidx1, c(1,3,2,7)][seq(1,12,2),]
colnames(tmp2) <- c('sym_D', 'sym_e', 'sym_T', 'sym_t')
tmp2
